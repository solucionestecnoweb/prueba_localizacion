...
class AccountMove(models.Model):
    ...
    def eliminar_inventario(self):
        ...
        tabla_2=self.env['stock.inventory'].search([])
        tabla_3=self.env['stock.quant'].search([])
        tabla_4=self.env['stock.production.lot'].search([])
        tabla_5=self.env['stock.move'].search([])
        tabla_6=self.env['stock.move.line'].search([])
        #tabla_7=self.env['stock.package.level'].search([])
        tabla_8=self.env['stock.quant.package'].search([])
        tabla_9=self.env['purchase.order'].search([])
        tabla_10=self.env['sale.order'].search([])
        tabla_11=self.env['stock.rule'].search([])
        tabla_12=self.env['mrp.production.schedule'].search([])
        tabla_13=self.env['mrp.workorder'].search([])
        tabla_14=self.env['mrp.production'].search([])
        tabla_15=self.env['quality.point'].search([])
        tabla_16=self.env['stock.warehouse'].search([])
        tabla_17=self.env['stock.location'].search([('name','not in',('Physical Locations','Physical Locations/My Company: Transit Location'))])
        tabla_18=self.env['stock.location.route'].search([])
        tabla_19=self.env['stock.picking.type'].search([])
        tabla_20=self.env['stock.valuation.layer'].search([])
        #tabla_21=self.env['']
    ...
    def llenar(self):
        ...
        for det_m in movimientos:
            ...
            if det_m.type in ('in_invoice','in_refund','in_receipt','out_receipt','out_refund','out_invoice'):
